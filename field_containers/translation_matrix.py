...
class TranslationMatrix(avango.script.Script):
  ...
  def constructor(self, json, app):
    ...
    values = json["values"]
    self.Vector.value = avango.gua.Vec3(values["Vector"][0], values["Vector"][1], values["Vector"][2])

    app.add_field_container(self) 

    for connection in json["field_connections"]:
      app.plan_field_connection(self.Name.value, connection["from_field"], connection["to_node"], connection["to_field"])
